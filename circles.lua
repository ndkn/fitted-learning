...
--This code doesn't use GPU. It takes about 15 minutes to train networks
require 'torch'
require 'nn'
require 'cunn'
require 'paths'
...
function transfrom_labels2(labelinds,numclass,ovcs) --separate ovc for each class
...
function transfrom_predict2(predict,numclass,ovcs)
  ...
  --predict=predict*ovc
  local tmppredict=torch.Tensor(sz,numclass):fill(1) --torch.ones(sz,numclass):cuda()
  local offset=0
  for j=1,numclass do
    for k=1,ovcs[j] do
...
image.save('circle_a.png',activationmap1)
